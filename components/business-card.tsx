...
export default function BusinessCard({ onExpand, isExpanded }: BusinessCardProps) {
  const handleHover = () => {
    console.log("[v0] Card hovered - isExpanded:", isExpanded)
    if (isExpanded) return
    console.log("[v0] Calling onExpand")
    onExpand?.()
  }

  console.log("[v0] BusinessCard render - isExpanded:", isExpanded)

  return (
    <div className="fixed inset-0 flex items-center justify-center bg-background p-4 z-50">
      <div className="cursor-pointer" onMouseEnter={handleHover}>
        <div
        ...
        >
          <h1 className="text-2xl font-serif font-bold mb-6 tracking-wide elegant-heading">KUSHAL AGRAWAL</h1>

          <div className="space-y-3 text-sm">
            <a
              href="mailto:kagrawal24@wisc.edu"
              className="text-muted-foreground block hover:text-foreground transition-colors duration-200 font-serif"
